...
export default class Navigation extends BaseControl {
    constructor(props) {
        super(props);
        this.navigableComponents = props.navigableComponents === undefined ? [] : props.navigableComponents;
        this.callback = props.callback;
        this.defaultNavigateTo = props.navigateTo === undefined ? this.navigableComponents.length / 2 : props.navigateTo;
        this.forwards = props.forwards === undefined ? null : true;
        this.backwards = props.backwards === undefined ? null : true;
        this.position = props.position === undefined ? ['50%', '0%'] : props.position;
        //how shallow should this component be in the view of the DOM tree?
        this.zIndex = props.zIndex === undefined ? 2050 : props.zIndex;
        this.forwardsColour = props.forwardsColour === undefined ? String('#474747') : props.forwardsColour;
        this.backwardsColour = props.backwardsColour === undefined ? String('#474747') : props.backwardsColour;
        this.noBorder = props.noBorder !== undefined;
        //determine border styling
        this.borderStyling = this.noBorder ? null: {border: `1px solid ${this.forwardsColour}`, borderRadius: 3};
         ...
         */
        this.isSetOfNavigation = this.forwards === undefined && this.backwards === undefined;
        this.finalBackwardsPosition = props.position === undefined || !this.isSetOfNavigation ? ['50%', '-0%'] : null;
        this.finalForwardsPosition = props.position === undefined || !this.isSetOfNavigation ? ['50%', '94%'] : null;
        //
    ...
    navigateTo = (itemIndex = this.defaultNavigateTo) => {
        ...
        if (currentComponentIndex === this.navigableComponents.length)
            ...
            currentComponentIndex = this.navigableComponents.length - 1;//last component
        else if (itemIndex < 0)
            //do swap
            currentComponentIndex = 0;
        this.setState(state => {
            state.currentComponent = currentComponentIndex;
            console.log('current state currentCompnent ', state.currentComponent);
            return state;
        });
        ...
        return itemIndex <= this.navigableComponents.length ? itemIndex
            :
    ...
    showForwards = (position = this.position) => {
        return (<div
            ...
            style={{position: 'absolute', top: position[0], left: position[1], zIndex: this.zIndex,...this.borderStyling}}>
            <svg onClick={() => {
    ...
    showBackwards = (position = this.position) => {
        return (<div
                ...
                style={{position: 'absolute', top: position[0], left: this.position[1], zIndex: this.zIndex, ...this.borderStyling}}>
                <svg onClick={() => {
                    this.navigateTo(this.state.currentComponent - 1);
                }}
                     xmlns="http://www.w3.org/2000/svg"
